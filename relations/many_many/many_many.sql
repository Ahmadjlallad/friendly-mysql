...
from series
         inner join reviews on series.id = reviews.reviewer_id
group by genre
...
select first_name,
       ...
       ifnull(max(rating), 0),
       ifnull(avg(rating), 0),
       IF(rating is not null, 'active', 'inactive')
from reviews
         right join reviewers on reviews.reviewer_id = reviewers.id
group by first_name, last_name, IF(rating is not null, 'active', 'inactive');
